...
train_dataloader = create_dataloader(train_df, batch_size=24)        ##################################batch_size
...
# from src.models.mamba.benchmarks.benchmark_generation_mamba_simple import device
device = "cuda"
sys.path.append('/home/cxw/Vim')
sys.path.append('/home/cxw/Vim/vim')
sys.path.append('/home/cxw/Vim/vim/mamba-1p1p1')
...
class AMBAModel(nn.Module):
    def __init__(self, label_dim=527,
        ...
        # use a pretrained models for finetuning
            ...
            audio_model = AMBAModel(fstride=p_fshape, tstride=p_tshape, fshape=p_fshape, tshape=p_tshape,
                                    ...
                                    model_size=model_size, vision_mamba_config=combined_vision_mamba_config)
    ...
    def finetuningavgtok_1sec(self, x):
        ...
        if not self.v.if_bidirectional:
            ...
            for i in range(len(self.v.layers) // 2):
                ...
                hidden_states_b, residual_b = self.v.layers[i * 2 + 1](hidden_states.flip([1]),
        ...
        x_averaged = torch.stack(
            [torch.mean(x[:, i * tokens_per_second:(i + 1) * tokens_per_second, :], dim=1) for i in range(60)], dim=1)
    ...
    def mpc(self, x, mask_patch, cluster, show_mask=False):
        ...
        pred = torch.empty((B, mask_patch, 256), device='cpu').float()  # e.g. size 12*100*768
        for i in range(B):
...
if __name__ == '__main__':
    ...
    ast_mdl = AMBAModel(label_dim=1,
                        fshape=16, tshape=16, fstride=10, tstride=10,
                        input_fdim=30, input_tdim=1997, model_size='base',
                        pretrain_stage=False, load_pretrained_mdl_path='./ssamba_base_400.pth', vision_mamba_config={
            'img_size': (128, 1024),
            'patch_size': 16,
            'stride': 8,
            'embed_dim': 768,
    ...
    test_input = torch.zeros([128, 1997, 30]).to(device)
...
def compute_eval_metrics(dataloader, model, log=True):
    ...
    for (x, y) in dataloader:
        ...
        pears = CovStep(pred, y, pred_mean, target_mean) / (
                    torch.sqrt(MSE(pred, pred_mean)) * torch.sqrt(MSE(y, target_mean)))
...
opt = Adam(model.parameters(), lr=0.0001, weight_decay=1e-5)

# 初始化学习率调度器
scheduler = StepLR(opt, step_size=20000, gamma=0.5)
...
for ep in range(15000):  #########################################################################
    ...
    for (x, y) in train_dataloader:
        (x, y) = (x.to(device), y.to(device))
        pred = model(x, task='ft_avgtok')
        loss = MSE(pred, y.reshape((y.shape[0], 1))).to(device)
    ...
    print(
        "Epoch: {}\tDuration: {:.2f}s\tTrain loss: {:.4f}\tVal loss: {:.4f}\tVal bias:{:.4f}\tVal Pearson correlation: {:.4e}\tVal MeanMult: {:.4f}" \
